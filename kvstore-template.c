...
int kvstore_spilt_token(char *msg,char **tokens){
    ...
    while(token != NULL){
        ...
        //继续分割下一个token
        token = strtok(NULL," ");
        //这里传NULL是为了告诉strtok函数继续从上一次分割的位置开始分割，而不是从字符串的开头重新分割。
    }
...
int kvstore_parse_protocol(struct conn_item *item,char **tokens,int count){
    ...
    switch(cmd){
        ...
        case KVS_CMD_DEL:{
            //del key
            int res = kvstore_array_del(tokens[1]);
            if(res < 0){
                snprintf(msg,BUFFER_LENGTH,"%s","ERROR");
            }else if(res == 0){
                snprintf(msg,BUFFER_LENGTH,"%s","SUCCESS");
        ...
        case KVS_CMD_MOD:{
            //mod key value
            int res = kvstore_array_mod(tokens[1],tokens[2]);
            if(res < 0){
                snprintf(msg,BUFFER_LENGTH,"%s","ERROR");
            }else if(res == 0){
                snprintf(msg,BUFFER_LENGTH,"%s","SUCCESS");
...
int kvstore_request(struct conn_item *item){
    ...
    for(idx = 0;idx < token_count;idx++){
        //这里的tokens数组中存储了分割出来的每个token,
        //例如对于"set key value"这个命令, tokens[0]就是"set", tokens[1]就是"key", tokens[2]就是"value"。
        printf("token: %s\n",tokens[idx]);
